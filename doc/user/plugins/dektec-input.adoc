...
With demodulator devices, the report includes the state of the input signal
(lock, strength, signal/noise ratio, bit error rate), as supported by the device.
Every statistic the device exposes is reported as its own JSON field (carrier and FEC
lock, CNR, MER, Es/N0, Eb/N0, link margin, per-stage BER/FER, etc.);
the `signal-locked`, `strength`, `snr` and `per` fields use the same names and formats as the `dvb` plugin.
